...
import io.milvus.param.index.IndexParam;
...
public class MilvusEmbeddingStore implements EmbeddingStore<TextSegment> {
    ...
    public void add(String id, Embedding embedding) {
        List<Float> vector = Arrays.asList(
            Arrays.stream(embedding.vector())
                .boxed()
                .toArray(Float[]::new)
        );
    ...
    public String add(Embedding embedding, TextSegment embedded) {
        ...
        String metadataJson = embedded.metadata().toMap().entrySet().stream()
        ...
        fields.add(new InsertParam.Field("id", Collections.singletonList(id)));
        fields.add(new InsertParam.Field("embedding", Collections.singletonList(vector)));
        fields.add(new InsertParam.Field("text", Collections.singletonList(embedded.text())));
    ...
    public List<String> addAll(List<Embedding> embeddings) {
        ...
        addAll(embeddings, Collections.emptyList());
    ...
    public List<String> addAll(List<Embedding> embeddings, List<TextSegment> embedded) {
        ...
        for (int i = 0; i < embeddings.size(); i++) {
            ...
            String text = embedded.size() > i ? embedded.get(i).text() : "";
            String metadata = embedded.size() > i ? 
                embedded.get(i).metadata().toMap().entrySet().stream()
                    .map(e -> "\"" + e.getKey() + "\":\"" + e.getValue() + "\"")
                    .collect(Collectors.joining(",", "{", "}")) : "{}";
    ...
    public EmbeddingSearchResult<TextSegment> search(EmbeddingSearchRequest request) {
        List<Float> queryVector = Arrays.asList(
            Arrays.stream(request.queryEmbedding().vector())
                .boxed()
                .toArray(Float[]::new)
        );
        
        Map<String, Object> searchParams = new HashMap<>();
        if ("HNSW".equals(config.getIndexType())) {
            searchParams.put("ef", config.getHnswEf());
        } else if ("IVF_FLAT".equals(config.getIndexType()) || "IVF_PQ".equals(config.getIndexType())) {
            searchParams.put("nprobe", config.getIvfNprobe());
        }
        ...
        if (response.getStatus() == R.Status.Success.getCode() && response.getData() != null) {
            List<io.milvus.param.search.SearchResultsWrapper.IDScore> idScores = 
                SearchResultsWrapper.getIDScore(response.getData().getResults().getTopK());
            
            for (io.milvus.param.search.SearchResultsWrapper.IDScore idScore : idScores) {
                double score = idScore.getScore();
                
                // 转换相似度分数（Milvus 返回距离，需要转换为相似度）
                double similarity = metricType == MetricType.L2 ? 
                    1.0 / (1.0 + score) : score;
                
                if (similarity >= request.minScore()) {
                    // 这里简化处理，实际应该从 metadata 中重建 TextSegment
                    TextSegment segment = TextSegment.from("");
                    matches.add(new EmbeddingMatch<>(similarity, idScore.getLongID(), null, segment));
                }
            }
        }
